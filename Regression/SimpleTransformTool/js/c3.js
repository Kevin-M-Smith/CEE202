(function (window) {
    ...
    c3_chart_internal_fn.getTooltipContent = function (d, defaultTitleFormat, defaultValueFormat, color) {
        ...
        for (i = 0; i < d.length; i++) {
        	...
        	if(d[i].id == "observed"){
        		var lev = hd[d[i].index];
            	text += "<tr class='" + CLASS.tooltipName + "-" + "leverage" + "'>";
            	text += "<td class='name'><span style='background-color:" + mp(lev) + "'></span>" + "leverage" + "</td>";
            	text += "<td class='value'>" + lev.toFixed(2) + "</td>";
            	text += "</tr>";
